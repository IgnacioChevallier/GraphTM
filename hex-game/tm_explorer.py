...
def new_exploration_args(current_index, permutate_exploration_params: bool = True):
    exploration_options = {
        "number_of_clauses": [100, 200, 1000],
        "s": [1.1, 3.0, 10.0],
        "T": [1, 10, 100, 1000],
...
'''
def explore_tms(starting_exploration_index, total_explorations):
    if total_explorations == 0:
        total_explorations = 100000000
    ...
    for i in range(total_explorations):
        ...
        number_of_nodes, node_names, games_train, games_test = setup_game.setup_game(args)
...
def main(single_run: bool = True, load_model: bool = True):
    ...
    if single_run:
        ...
        explore_tms(random.randint(0,10**10), 0)
