...
from edge_strategies import build_edge_neighbor_map
...
class graph_tm:
    ...
    def create_graphs_nodes(self, graphs, number_of_graphs, number_of_nodes):
        if self.edge_neighbor_map is None:
            raise RuntimeError("Edge neighbor map must be prepared before creating nodes.")
        ...
        for graph_id in range(number_of_graphs):
            for node_name in self.node_names:
                number_of_outgoing_edges = len(self.edge_neighbor_map.get(node_name, []))
                graphs.add_graph_node(graph_id, node_name, number_of_outgoing_edges)


    '''
    Creating edges from the selected edge strategy.
    '''
    def create_graphs_edges(self, graphs, number_of_graphs, number_of_nodes):
        if self.edge_neighbor_map is None:
            raise RuntimeError("Edge neighbor map must be prepared before creating edges.")

        graphs.prepare_edge_configuration()

        for graph_id in range(number_of_graphs):
            for node_name, neighbors in self.edge_neighbor_map.items():
    ...
    def run(self):
        ...
        # Build edge neighbor map according to the selected topology before creating nodes/edges
        self.edge_neighbor_map = build_edge_neighbor_map(self.node_names, self.edge_connections)
